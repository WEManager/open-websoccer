...
use App\Classes\Services\BadgesDataService;
use App\Classes\Services\BankAccountDataService;
use App\Classes\Services\NotificationsDataService;
use App\Classes\Services\TeamsDataService;
...
class DataUpdateSimulatorObserver implements ISimulatorObserver {
	...
	private function notifyAboutSoonEndingContracts($userId, $teamId) {
		NotificationsDataService::createNotification(
			$this->_websoccer,
			$this->_db, $userId,
			'notification_soon_ending_playercontracts',
			'',
			'soon_ending_playercontracts',
			'myteam',
			null,
			$teamId
		);
