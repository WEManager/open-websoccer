...
class ViewHandler {
	 ...
	 */
	public function __construct(WebSoccer $website, DbConnection $db, I18n $i18n, &$pages, &$blocks, $validationMessages = null) {
	...
	public function renderBlock(string $blockId, array $viewConfig = null, array $parameters = null) {
		if ($viewConfig === null) {
	...
	private function _getBlocksForPage($pageId) {
		...
		foreach($this->_blocks as $blockData) {
		...
		foreach($blocks as $uiblock => $blockdata) {
			if ($uiblock != 'custom') {
				usort($blocks[$uiblock], array('App\Classes\ViewHandler', 'sortByWeight'));
			}
		}
